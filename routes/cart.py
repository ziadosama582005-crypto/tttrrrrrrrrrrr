...
def checkout_page():
    ...
    # حساب إجمالي السلة مع جلب الأسعار الحقيقية من Firebase
    total = 0
    cart_items = []
    for item in cart.get('items', []):
        ...
        if product_doc.exists:
            ...
            if not product.get('sold', False):
                ...
                total += price
                cart_items.append({
                    'name': product.get('item_name', item.get('name', 'منتج')),
                    'price': price
                })
...
@cart_bp.route('/api/checkout/data', methods=['GET'])
def get_checkout_data():
    """جلب بيانات الدفع الحقيقية من Firebase"""
    user_id = session.get('user_id')
    if not user_id:
        return jsonify({'status': 'error', 'message': 'لم يتم تسجيل الدخول'}), 401
    ...
    try:
        ...
        print(f"❌ خطأ في جلب بيانات الدفع: {e}")
